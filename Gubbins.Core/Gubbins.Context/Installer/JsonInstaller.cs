...
/// <summary>
/// Json installer implementation.
/// </summary>
/// <remarks>
/// Json installer is used to deserialize and store configuration data from a Json file. The Json format is as follows:
/// <code>
/// [
///     {
///         "key": "{Key_Name}",
...
/// </code>
/// </remarks>
public sealed class JsonInstaller(string json) : IDependenciesInstaller
{
    public void Install(IDependenciesRegistry registry) => registry.RegisterAll(Deserialize());

    /// <summary>
    /// Json deserializer.
    /// </summary>
    private IEnumerable<InstallInfo> Deserialize()
    {
        var results = new List<InstallInfo>();

        foreach (var element in SplitJsonObjects(json.Trim().Trim('[', ']').Trim()))
        {
