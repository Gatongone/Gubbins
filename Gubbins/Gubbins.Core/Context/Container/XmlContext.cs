...
/// <summary>
/// Xml context implementation.
/// </summary>
/// <remarks>
/// Xml context is used to deserialize and store configuration data from an XML file. The XML format is as follows:
/// <code>
...
/// </code>
/// </remarks>
public sealed class XmlContext : ConfigContext<XmlDocument>
{
    /// <summary>
    /// Xml deserializer.
    /// </summary>
    private static readonly IDeserializer<XmlDocument> s_Formatter = new XmlFormatter();

    /// <param name="config">Xml document.</param>
    /// <param name="parent">Parent context.</param>
    public XmlContext(XmlDocument config, IContext? parent = null) : base(config, s_Formatter, parent) { }

    /// <param name="configs">Xml documents.</param>
    /// <param name="parent">Parent context.</param>
    public XmlContext(IEnumerable<XmlDocument> configs, IContext? parent = null) : base(configs, s_Formatter, parent) { }

    /// <summary>
    /// Xml deserializer.
    /// </summary>
    private class XmlFormatter : IDeserializer<XmlDocument>
    {
        private static readonly HashSet<Type> s_EmptyBindings;

        public TTarget Deserialize<TTarget>(XmlDocument doc)
        {
            ...
            static InstallInfo BuildInstallInfo(XmlElement element)
            {
                ...
                result.Bindings   = bindings == null ? s_EmptyBindings : bindings.Select(binding => Type.GetType(binding.Attributes["type"]!.Value, Reflection.LoadAssemblyResolver, Reflection.DomainTypeResolver)).ToHashSet();
                return result;
            }
        }
    }
